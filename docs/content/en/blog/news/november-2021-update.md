...
Until now, PipeCD has been providing a way to allow the developer to configure a list `query` and `threshold` to analyze metrics data of a running deployment. Any query that returns a result exceeding the given threshold will cause the deployment to be failed, and a rollback stage will be executed if needed. Although this way is relatively straightforward, there is a challenge you can see here; it isn't easy to find an appropriate threshold value to use. And software is constantly changing time over time that making those values could be changed dynamically. Therefore, some new analysis strategies `PREVIOUS`, `CANARY_BASELINE`, `CANARY_PRIMARY` have been introduced since `v0.20.0`. They allow analyzing without requiring any threshold values.
...
If you have any feature want to request or found a problem, please let us know by creating an issue in the [pipe-cd/pipe](https://github.com/pipe-cd/pipe/issues) repository.
