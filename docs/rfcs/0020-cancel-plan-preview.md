...
In this iteration, a cancel plan preview will not be saved in the database/firestore. Therefore a cancel plan preview command can be thought of as a clean up action to cancel plan preview commands that have not yet started executing.

The taarget version of piped agent for the cancel plan preview command is version 0. In the future we want to would explore having a generic cancel command which would target piped version 1.
