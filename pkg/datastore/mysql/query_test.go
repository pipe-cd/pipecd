...
func TestBuildFindQuery(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			expectedQuery: "SELECT Data FROM Application WHERE ProjectId = ? AND UpdatedAt <= ? AND Id > ? ORDER BY UpdatedAt DESC, Id ASC LIMIT 20",
			wantErr:       false,
		},
		{
			...
			expectedQuery: "SELECT Data FROM Application WHERE UpdatedAt <= ? AND Id > ? ORDER BY UpdatedAt DESC, Id ASC",
			wantErr:       false,
		},
		{
