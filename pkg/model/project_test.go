...
func Test_VerifyRBACPermission(t *testing.T) {
	type args struct {
		typ       ProjectRBACResource_ResourceType
	...
	testcases := []struct {
	...
	}{
		{
			name: "ok",
			args: args{
				typ:       ProjectRBACResource_PROJECT,
				action:    ProjectRBACPolicy_GET,
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			got := VerifyRBACPermission(tc.args.typ, tc.args.action, tc.args.userRoles, tc.args.roles)
