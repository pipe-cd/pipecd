...
func (m *Deployment) validate(all bool) error {
	...
	{
		sorted_keys := make([]string, len(m.GetPluginMetadata()))
		i := 0
		for key := range m.GetPluginMetadata() {
			sorted_keys[i] = key
			i++
		}
		sort.Slice(sorted_keys, func(i, j int) bool { return sorted_keys[i] < sorted_keys[j] })
		for _, key := range sorted_keys {
			val := m.GetPluginMetadata()[key]
			_ = val

			// no validation rules for PluginMetadata[key]
			...
			if all {
				...
				case interface{ ValidateAll() error }:
					if err := v.ValidateAll(); err != nil {
						errors = append(errors, DeploymentValidationError{
							field:  fmt.Sprintf("PluginMetadata[%v]", key),
							reason: "embedded message failed validation",
							cause:  err,
						})
					}
