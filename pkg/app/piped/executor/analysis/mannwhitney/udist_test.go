...
func TestUDistTies(t *testing.T) {
	...
	makeTable := func(m, N int, _t []int, minx, maxx float64) [][]float64 {
		out := [][]float64{}
		dist := UDist{N1: m, N2: N - m, T: _t}
...
func udistRef(t *testing.T, n1 int, _t []int) (pmf, cdf []float64) {
	t.Helper()
	...
	counts := make([]int, 1+2*n1*(sumint(_t)-n1))
...
enumu:
	for {
		...
		prod := 1
		for i, u_i := range u {
			prod *= int(mathChoose(_t[i], u_i) + 0.5)
		}
	...
	total := int(mathChoose(sumint(_t), n1) + 0.5)
...
func printUmemo(t *testing.T, A []map[ukey]float64, _t []int) {
	t.Helper()

	fmt.Printf("K\tn1\t2*U\tpr\n")
	for K := len(A) - 1; K >= 0; K-- {
		for i, pr := range A[K] {
			_, ref := udistRef(t, i.n1, _t[:K])
