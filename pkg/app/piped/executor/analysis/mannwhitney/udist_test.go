...
func TestUDistTies(t *testing.T) {
	...
	makeTable := func(m, N int, t_ []int, minx, maxx float64) [][]float64 {
		out := [][]float64{}
		dist := UDist{N1: m, N2: N - m, T: t_}
...
func udistRef(t *testing.T, n1 int, t_ []int) (pmf, cdf []float64) {
	t.Helper()
	...
	counts := make([]int, 1+2*n1*(sumint(t_)-n1))
...
enumu:
	for {
		...
		prod := 1
		for i, u_i := range u {
			prod *= int(mathChoose(t_[i], u_i) + 0.5)
		}
	...
	total := int(mathChoose(sumint(t_), n1) + 0.5)
...
func printUmemo(t *testing.T, A []map[ukey]float64, t_ []int) {
	t.Helper()

	fmt.Printf("K\tn1\t2*U\tpr\n")
	for K := len(A) - 1; K >= 0; K-- {
		for i, pr := range A[K] {
			_, ref := udistRef(t, i.n1, t_[:K])
