...
type FormAction =
  ...
  | { type: "reset" };
export type AddApplicationDrawerState = Record<FormKey, string> & {
  kind: ApplicationKind;
  repo: ApplicationGitRepository.AsObject;
};
function reducer(
  state: AddApplicationDrawerState,
  action: FormAction
): AddApplicationDrawerState {
  switch (action.type) {
    case "update-piped":
    ...
    case "update-env":
      // NOTE: clear values that references to env.
      return {
        ...initialState,
        env: action.value,
        kind: state.kind,
...
interface Props {
  ...
  isAdding: boolean;
  projectName: string;
  onSubmit: (state: AddApplicationDrawerState) => void;
  onClose: () => void;
}

export const AddApplicationDrawer: FC<Props> = ({
  open,
  isAdding,
  projectName,
  onSubmit,
        ...
        <form className={classes.form} onSubmit={handleSubmit}>
          ...
          <div className={classes.inputGroup}>
            ...
            <FormSelectInput
              ...
              disabled={isAdding || !formState.env || pipeds.length === 0}
            />
          </div>
