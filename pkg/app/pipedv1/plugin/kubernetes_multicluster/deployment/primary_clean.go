...
func (p *Plugin) executeK8sMultiPrimaryCleanStage(ctx context.Context, input *sdk.ExecuteStageInput[kubeconfig.KubernetesApplicationSpec], dts []*sdk.DeployTarget[kubeconfig.KubernetesDeployTargetConfig]) sdk.StageStatus {
	...
	for _, tc := range targetConfigs {
		eg.Go(func() error {
			lp.Infof("Start cleaning PRIMARY variant on target %s", tc.deployTarget.Name)
			if err := p.primaryClean(ctx, input, tc.deployTarget, cfg); err != nil {
				return fmt.Errorf("failed to clean PRIMARY variant on target %s: %w", tc.deployTarget.Name, err)
			}
			return nil
		})
	}
	...
	if err := eg.Wait(); err != nil {
		lp.Errorf("Failed while cleaning PRIMARY variant (%v)", err)
		return sdk.StageStatusFailure
	}
...
func (p *Plugin) primaryClean(
	...
	if err := deleteVariantResources(ctx, lp, kubectl, dt.Config.KubeConfigPath, applier, input.Request.Deployment.ApplicationID, variantLabel, primaryVariant); err != nil {
		return fmt.Errorf("unable to remove primary resources: %w", err)
	}
