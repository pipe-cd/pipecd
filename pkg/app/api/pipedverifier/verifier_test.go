...
func (g *fakeProjectGetter) GetProject(ctx context.Context, id string) (*model.Project, error) {
	g.calls++
	p, ok := g.projects[id]
	if ok {
		msg := proto.Clone(p)
...
func (g *fakePipedGetter) GetPiped(ctx context.Context, id string) (*model.Piped, error) {
	g.calls++
	p, ok := g.pipeds[id]
	if ok {
		msg := proto.Clone(p)
