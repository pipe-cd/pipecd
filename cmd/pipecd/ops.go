...
func (s *ops) run(ctx context.Context, input cli.Input) error {
	...
	insightMetricsCollector := insightmetrics.NewInsightMetricsCollector(insightstore.NewStore(fs), datastore.NewProjectStore(ds, datastore.OpsWriter))

	// Start running HTTP server.
	{
		handler := handler.NewHandler(s.httpPort, datastore.NewProjectStore(ds, datastore.OpsWriter), insightstore.NewStore(fs), cfg.SharedSSOConfigs, s.gracePeriod, input.Logger)
		group.Go(func() error {
